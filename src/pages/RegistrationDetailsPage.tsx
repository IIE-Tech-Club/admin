...
export function RegistrationDetailsPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        const regRes = await fetch(
          `${import.meta.env.VITE_API_URL}/registrations/detail/${registrationId}`,
        );
  ...
  const handleDelete = async () => {
    if (
      !window.confirm(
        "CRITICAL: You are about to purge this entity from the registry. This action is irreversible. Proceed?",
      )
    )
      return;
    ...
    try {
      ...
      const msg =
        error instanceof Error ? error.message : "Internal System Failure";
  ...
  if (!registration) {
    return (
      <div className="text-center p-20">
        <h2 className="text-2xl font-black text-rose-500 font-orbitron mb-4">
          ENTITY NOT FOUND
        </h2>
        <Link
          to="/h/$hackathonId/registrations"
          params={{ hackathonId }}
          className="neon-btn-cyan !px-8"
        >
          BACK TO REGISTRY
        </Link>
      </div>
      ...
      <header>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] font-black text-cyan-400 font-orbitron uppercase tracking-widest mb-1">
                Identity File
              </p>
              <h1 className="text-2xl sm:text-4xl font-black text-white font-orbitron tracking-tight uppercase truncate">
                {getParticipantName()}
              </h1>
            </div>
          </div>
      ...
      <div className="grid gap-8 md:grid-cols-3">
        ...
        <aside className="md:col-span-1 space-y-6">
          <article className="glass-card p-6 border-cyan-500/20">
            <h3 className="text-xs font-black text-white font-orbitron uppercase tracking-widest mb-6 border-b border-white/5 pb-4">
              Core Identity
            </h3>

            <div className="space-y-6">
              <div>
                <p className="text-[9px] font-black text-slate-500 font-orbitron uppercase tracking-widest mb-1">
                  Email Address
                </p>
          ...
          {teamName && (
            <article className="glass-card p-6 border-cyan-500/30 bg-cyan-500/5">
              <h3 className="text-[10px] font-black text-cyan-400 font-orbitron uppercase tracking-widest mb-4">
                Squad Linkage
              </h3>
              ...
              >
                <span className="text-xs font-black text-white font-orbitron uppercase tracking-wider">
                  {teamName}
                </span>
          ...
          <div className="pt-6 border-t border-white/5">
            ...
            <p className="text-[8px] text-slate-600 font-mono mt-2 text-center uppercase tracking-widest">
            ...
            <div className="space-y-8">
              {(() => {
                const registrationPhase = hackathon?.phases?.find(
                  (p) => p.id === "phase_1_registration",
                );
                if (!registrationPhase)
                  return (
                    <p className="text-xs font-mono text-slate-500 italic">
                      Phase [phase_1_registration] not located in database.
                    </p>
                  );

                const phaseResponse =
                  registration.responses?.[registrationPhase.id];
                if (!phaseResponse)
                  return (
                    <div className="opacity-30 p-6 border border-white/5 grayscale">
                      <p className="text-[10px] font-black text-slate-500 font-orbitron uppercase tracking-widest">
                        Phase: {registrationPhase.name} [PENDING]
                      </p>
                    </div>
                  );
                  ...
                  <div className="p-6 bg-slate-900/40 border border-cyan-500/10 hover:border-cyan-500/30 transition-all group">
                    <div className="flex items-center justify-between mb-6 border-b border-white/5 pb-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[9px] font-mono text-cyan-400 bg-cyan-400/5 px-2 py-0.5 border border-cyan-400/20">
                          MANIFEST_VERIFIED
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="grid gap-6 md:grid-cols-2">
                      {registrationPhase.fields?.map((field) => {
                        const val =
                          typeof phaseResponse === "object" &&
                          phaseResponse !== null
                            ? (phaseResponse as Record<string, unknown>)[
                                field.id
                              ]
                            : phaseResponse;
                        ...
                        return (
                          <div
                            key={field.id}
                            className={
                              field.type === "file" ? "col-span-2" : ""
                            }
                          >
                            <div className="flex items-center gap-2 mb-2">
                              <span className="h-1 w-1 bg-slate-700" />
                              <p className="text-[9px] font-black text-slate-500 font-orbitron uppercase tracking-widest">
                                {field.label}
                              </p>
                            </div>
                            {field.type === "file" ? (
                              <div className="flex items-center gap-4 p-4 bg-slate-950 border border-white/5 group-hover:border-cyan-500/20 transition-colors">
                                ...
                                <div className="flex-1 min-w-0">
                                  <p className="text-[10px] font-black text-white font-orbitron uppercase tracking-widest mb-1">
                                    DATA_OBJECT_ATTACHMENT
                                  </p>
                                  <p className="text-[9px] font-mono text-slate-500 truncate opacity-60">
                                    {String(val || "NO_PATH_LOCATED")}
                                  </p>
                                </div>
                                {!!val && (
                                  <a
                      ...
                      {(!registrationPhase.fields ||
                        registrationPhase.fields.length === 0) && (
                        <div className="col-span-2 py-4 text-[10px] text-slate-600 font-mono italic flex items-center gap-2">
                          <span className="text-cyan-500/50">▶</span> Sequence
                          parameters satisfied. No additional telemetry
                          recorded.
                        </div>
                      )}
                    </div>
